          ...
          <thead>
            <tr>
              ...
              {[...Array(maxCycle)].map((_, i) => (
                <th key={i} className="border p-2 w-12">{i + 1}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {instructions.map(inst => (
              <tr key={inst.id}>
                <td className="border p-2 font-mono">{inst.text}</td>
                {[...Array(maxCycle)].map((_, cycle) => {
                  ...
                  return (
                    <td 
                      key={cycle}
                      className={`border p-2 text-center ${getStageColor(stage, stall)}`}
                    >
                      {stall ? `S${stall}` : stage}
                    </td>
                  );
      ...
      <div className="mb-4 md:mb-6">
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse text-sm md:text-base">
            <thead>
              <tr>
                <th className="border p-2 md:p-3">Instruction</th>
                <th className="border p-2 md:p-3">Comment</th>
                {[...Array(maxCycle)].map((_, i) => (
            ...
            <tbody>
              {instructions.map(inst => (
                <tr key={inst.id}>
                  <td className="border p-2 md:p-3 font-mono whitespace-nowrap">{inst.text}</td>
                  <td className="border p-2 md:p-3 text-xs md:text-sm text-gray-600">{inst.comment || ''}</td>
                  {[...Array(maxCycle)].map((_, cycle) => {
                    const instState = pipelineState.get(inst.id) || {};
                    const stallInfo = stallState.get(inst.id) || {};
                    const stage = getStageAtCycle(instState, cycle + 1);
